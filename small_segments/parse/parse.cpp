...
int process_transaction(pugi::xml_document &request_doc, pugi::xml_document &response_doc,connection *C) {
  ...
  for (pugi::xml_node child : request_doc.child("transactions")) {
    ...
    if(to_string(child.name()) == "order")
    {
      ...
      pugi::xml_parse_result order_node = response_doc.append_buffer(order.c_str(), order.length());
    }
    else if(to_string(child.name()) == "query")
    {
      int trans_id = getTrans_ID(child);
      std :: string query = query(C, trans_id);
      pugi::xml_parse_result query_node = response_doc.append_buffer(query.c_str(), query.length());
    }
    else if(to_string(child.name()) == "cancel")
    {
      int trans_id = getTrans_ID(child);
      string canceled = cancel(C, trans_id);
      pugi::xml_parse_result canceled_node = child.append_buffer(canceled.c_str(), canceled.length());
    }
    else
    {
